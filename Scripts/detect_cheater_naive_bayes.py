...
from models import KDENaiveBayes
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python detect_cheater_naive_bayes.py <player_name>")
        sys.exit(1)
    ...
    print("[INFO] Selecting 'overview' features...")
    # Load exact features used in training
    overview_features = joblib.load("kde_overview_feature_names.pkl")
    ...
    df = df[overview_features]

    bool_cols = df.select_dtypes(include=["bool"]).columns
    df[bool_cols] = df[bool_cols].astype(int)

    print("[INFO] Loading model and making prediction...")
    model = joblib.load("kde_naive_bayes_model.pkl")
